...
\caption{Three-stage PySpark pipeline architecture with HDFS storage and shared utilities.}
...
\textbf{Example: Full pipeline execution and output.} The complete pipeline can be executed with Docker in a single command:

\begin{verbatim}
docker compose up --build
\end{verbatim}

This provisions a Spark environment, loads all 18 datasets into HDFS, runs the three-stage pipeline (data preparation, analytics, visualization), and outputs the interactive dashboard to the \texttt{output/} directory. Alternatively, users can run individual stages via \texttt{scripts/run\_pipeline\_local.sh}.

The analytics stage produces several outputs: (i)~KMeans cluster assignments grouping states by crime profiles---for instance, Uttar Pradesh and Madhya Pradesh fall into a high-crime cluster while northeastern states form a low-crime cluster; (ii)~a women safety index computed as an inverse normalized score from crimes-against-women data; and (iii)~state-wise forecasts for 2015--2020 using model selection between Ridge polynomial regression and gradient boosting with time-series cross-validation.

The visualization stage generates an interactive choropleth map of India (Figure~\ref{fig:national_overview}) where states are colored by crime intensity. Users can hover over any state to view detailed statistics, toggle between crime categories, and navigate the 2001--2014 time period using a year selector. The state mapping module automatically resolves naming inconsistencies across datasets---for example, mapping ``Delhi UT'', ``Nct Of Delhi'', and ``Delhi'' to a canonical ``Delhi'' entry.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{01_national_overview.png}
\caption{National overview dashboard showing the interactive choropleth map with state-level crime statistics and year selector.}
\label{fig:national_overview}
\end{figure}
